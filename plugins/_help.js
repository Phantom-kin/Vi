...
➮Fᴏᴜɴᴅᴇʀ- *𝕻𝖍𝖆𝖓𝖙𝖔𝖒*
➮Co-founder- *STAR-KING0*
➮Oᴡɴᴇʀ - ${Config.ownername} 
➮Nᴜᴍ - ${owner.split(",")[0]} 
➮Mᴇᴍᴏ - ${formatp(os.totalmem() - os.freemem())} 
      \n *ℭ𝔬𝔰𝔪𝔬-ℌ𝔲𝔫𝔱𝔢𝔯 𝔞 𝔰𝔦𝔪𝔭𝔩𝔢 𝔚𝔥𝔞𝔱𝔰𝔄𝔭𝔭 𝔟𝔬𝔱*\n\n ${readmore} 
╭──❰ *ALL MENU* ❱ 
│⚔️ Lɪꜱᴛ 
│⚔️ Cᴀᴛᴇɢᴏʀʏ 
│⚔️ Hᴇʟᴘ 
...
smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("⚔️ Cosmo-Hunter ⚔️");

    // Wait briefly for effect
    await new Promise((r) => setTimeout(r, 1500));

    var speed = new Date().getTime() - startTime;
    await message.send(`⚔️ 𝕶𝖎𝖓𝖌 𝕻𝖍𝖆𝖓𝖙𝖔𝖒'𝖘 𝕸𝖆𝖓𝖆: ${speed} ms ⚡`, { edit: key });
  }
);
astro_patch.cmd(
  {
